...
public class PatchDatagen {
	...
	public static void gatherData(GatherDataEvent event) {
		...
		if (event.includeServer()) {
			...
			generator.addProvider(event.includeServer(), provider = new PatchBlockTags(packOutput, lookupProvider, existingFileHelper));
			generator.addProvider(event.includeServer(), new PatchItemTags(packOutput, lookupProvider, provider, existingFileHelper));
		}
		if (event.includeClient()) {
			generator.addProvider(event.includeClient(), new Language(packOutput));
			generator.addProvider(event.includeClient(), new BlockModels(packOutput, existingFileHelper));
	...
	private static class Language extends LanguageProvider {
		public Language(PackOutput packOutput) {
			super(packOutput, FlowerPatch.MOD_ID, "en_us");
		}
	...
	private static class BlockStates extends BlockStateProvider {
		public BlockStates(PackOutput packOutput, ExistingFileHelper helper) {
			super(packOutput, FlowerPatch.MOD_ID, helper);
		}
	...
	private static class BlockModels extends BlockModelProvider {
		...
		private void crossBlock(Block block) {
			patchBlock(block, 2).renderType("cutout");
			patchBlock(block, 3).renderType("cutout");
			patchBlock(block, 4).renderType("cutout");
		}
	...
	public static class PatchBlockTags extends BlockTagsProvider {
		...
		public PatchBlockTags(PackOutput packOutput, CompletableFuture<HolderLookup.Provider> lookupProvider, @Nullable ExistingFileHelper existingFileHelper) {
			super(packOutput, lookupProvider, FlowerPatch.MOD_ID, existingFileHelper);
		}
	...
	public static class PatchItemTags extends ItemTagsProvider {
		public PatchItemTags(PackOutput packOutput, CompletableFuture<HolderLookup.Provider> lookupProvider, BlockTagsProvider blockTagsProvider, ExistingFileHelper existingFileHelper) {
			super(packOutput, lookupProvider, blockTagsProvider, FlowerPatch.MOD_ID, existingFileHelper);
		}
